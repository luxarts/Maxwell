...
void Printer::showJSONStatus(int type) {
	...
	/*Maxwell-start*/
	Com::printF(PSTR("JSONStatus{\"temp\":{\"extruder\": {\"actual\":"));
	Com::printFloat(extruder[0].tempControl.currentTemperatureC, 1);//temp.extruder.actual = 0.0 / 300.0
	Com::printF(PSTR(",\"target\":"));
	Com::print(extruder[0].tempControl.targetTemperatureC);		//temp.extruder.target = 0 / 300
	Com::printF(PSTR("},\"bed\":{\"actual\":"));
	Com::printFloat(heatedBedController.currentTemperatureC, 1);//temp.bed.actual = 0.0 / 130.0
	Com::printF(PSTR(",\"target\":"));
	Com::print(heatedBedController.targetTemperatureC);			//temp.bed.target = 0 / 130
	...
	if(PrintLine::linesCount == 0) {
		...
		if(sd.sdmode == 1){                                       
			...
			Com::print("\"p\"");                                //printing.status = p (paused) 
		}
	}
	else{
		Com::print("\"E\"");									//printing.status = E (something else)
	}
	Com::printF(PSTR(",\"percent\":"));
	Com::print(Printer::progress);								//printing.percent = 0 / 100
	Com::printF(PSTR(",\"filename\":\""));
	Com::print(Printer::printName);								//printing.filename = "archivo.gcode"
	Com::printF(PSTR("\",\"time\":"));
	Com::print(Printer::msecondsPrinting);						//printing.time = 0 / 4294967295 (49 days)
	Com::printF(PSTR("},\"pos\":{\"x\":"));
	Com::print(currentPosition[X_AXIS]);						//pos.x = -300 / 300
	Com::printF(PSTR(",\"y\":"));
	Com::print(currentPosition[Y_AXIS]);						//pos.y = -300 / 300
	Com::printF(PSTR(",\"z\":"));
	Com::print(currentPosition[Z_AXIS]);						//pos.z = 0 / 500
	Com::printF(PSTR(",\"homed\":"));
	if((int)isXHomed() && isYHomed() && isZHomed()){
		Com::print((int)1);											//pos.homed = 1
	}
	else{
		Com::print((int)0);											//pos.homed = 0
	}
	Com::printF(PSTR("},\"caselight\":"));
	Com::print(READ(CASE_LIGHTS_PIN));							//caselight = 0 / 1
	Com::printFLN(PSTR("}"));
	/*Maxwell-end*/

	/*	Com::printF(PSTR("{\"printing\": \""));
	if (PrintLine::linesCount == 0) {
		Com::print('I'); // IDLING
#if SDSUPPORT
	} else if (sd.sdactive) {
