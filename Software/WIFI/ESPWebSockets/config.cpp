...
/*void CONFIG_writeString(int pos, String string_Buffer){
  int size_buffer;
  size_buffer = strlen(string_buffer);//Cantidad de caracteres

  EEPROM.begin(EEPROM_SIZE);//Inicia la EEPROM
  
  for (int i = 0; i < size_buffer; i++) {
    EEPROM.write(pos + i, string_buffer.charAt(i));//Escribe todos los caracteres del vector
  }
...
void CONFIG_readBuffer(int pos, char * byte_buffer, int maxLength){
  uint8_t b=0;
...
void CONFIG_reset(void){
  ...
  CONFIG_writeBuffer(EP_STA_SSID, D_STA_SSID);
  CONFIG_writeBuffer(EP_STA_PASSWORD, D_STA_PASSWORD);
}
